...
void Shapes::BuildGeometry()
{
    ...
    graphics->Copy(geom.VertexData(), vbSize, mesh->vertexBufferCPU);
    graphics->Copy(geom.IndexData(), ibSize, mesh->indexBufferCPU);

    // copia vértices e índices para a GPU usando o buffer de Upload
    graphics->Copy(geom.VertexData(), vbSize, mesh->vertexBufferUpload, mesh->vertexBufferGPU);
    graphics->Copy(geom.IndexData(), ibSize, mesh->indexBufferUpload, mesh->indexBufferGPU);
}
